...
function uploadImg() {
    ...
    formData.append('_via_wrong_img_list[]', wrong_img_list);
...
function _set_attributes() {
    ...
    $("#button_show_file_attributes").click(function () {
        ...
        if (firstTry < 2) {
            $("#user_input_attribute_id").val('Fx_region');
            $("#button_add_new_attribute").click();

            attribute_property_on_update(document.getElementById('attribute_type'))
            document.getElementById('attribute_type').options[4].removeAttribute('selected')
            document.getElementById('attribute_type').options[1].setAttribute('selected', 'selected')
            attribute_property_on_update(document.getElementById('attribute_type'))

            document.getElementById('_via_attribute_new_option_id').setAttribute('value', 'radius')
            attribute_property_on_option_add(document.getElementById('_via_attribute_new_option_id'))
            document.getElementById('_via_attribute_new_option_id').setAttribute('value', 'scaphoid')
            attribute_property_on_option_add(document.getElementById('_via_attribute_new_option_id'))
            document.getElementById('_via_attribute_new_option_id').setAttribute('value', 'styloid')
            attribute_property_on_option_add(document.getElementById('_via_attribute_new_option_id'))
        }
    });
...
function update_vertical_space() {
    ...
    var bepanel = document.getElementById('annotation_editor_panel2');
    panel.style.height = (aepanel.offsetHeight + 40) + 'px';
    panel.style.height = (bepanel.offsetHeight + 40) + 'px';
}
...
function attribute_update_panel_set_active_button() {
    if (firstTry === 0 || firstTry === 3) {   // new project 일때만 attributes 추가, 프로젝트 저장하면 입력한 attrs들이 json 파일에 추가되므로 프로젝트 로드할 때는 함수를 다시 호출할 필요 없음
        _set_attributes();
    }
...
function annotation_editor_show() {
    ...
    be.setAttribute('id', 'annotation_editor2');
    if (_via_annotation_editor_mode === VIA_ANNOTATION_EDITOR_MODE.SINGLE_REGION) {
        ...
        console.log(_via_metadata_being_updated);

        _via_annotaion_editor_panel.appendChild(ae);
        _via_annotaion_editor_panel2.appendChild(be);
...
function annotation_editor_update_content() {
    return new Promise(function (ok_callback, err_callback) {
        ...
        var be = document.getElementById('annotation_editor2');
        ...
        if (be) {
            be.innerHTML = '';
            annotation_editor_update_header_html2();
            annotation_editor_update_metadata_html2();
        }
        ok_callback();
    });
}
...
function annotation_editor_remove() {
    ...
    var q = document.getElementById('annotation_editor2');
    if (q) {
        q.remove();
    }
}
function is_annotation_editor_visible2() {
    console.log('visible2')
    return document.getElementById('annotation_editor_panel2').classList.contains('display_block');
}
...
$(function () {
    ...
    var q = document.getElementById('annotation_editor_panel2');
    _via_annotation_editor_mode = VIA_ANNOTATION_EDITOR_MODE.ALL_REGIONS;
    q.classList.add('display_block');
    q.style.height = _via_settings.ui.annotation_editor_height + '%';
    q.style.fontSize = _via_settings.ui.annotation_editor_fontsize + 'rem';
    annotation_editor_show();
});
function annotation_editor_toggle_all_regions_editor() {
    ...
    var q = document.getElementById('annotation_editor_panel2');
    if (q.classList.contains('display_block')) {
        q.classList.remove('display_block');
        _via_annotation_editor_mode = VIA_ANNOTATION_EDITOR_MODE.SINGLE_REGION;
    } else {
        _via_annotation_editor_mode = VIA_ANNOTATION_EDITOR_MODE.ALL_REGIONS;
        q.classList.add('display_block');
...
function annotation_editor_update_header_html2() {
    var head = document.createElement('div');
    head.setAttribute('class', 'row');
    head.setAttribute('id', 'annotation_editor_header2');
    ...
    if (be.childNodes.length === 0) {
        ...
        if (be.firstChild.id === 'annotation_editor_header2') {
            be.replaceChild(head, be.firstChild);
        } else {
            // header node is absent
            be.insertBefore(head, be.firstChild);
        }
    }
}
...
function annotation_editor_update_metadata_html2() {
    if (!_via_img_count) {
        return;
    }

    var be = document.getElementById('annotation_editor2');
    be.appendChild(annotation_editor_get_metadata_row_html2(0));
}
function annotation_editor_update_row(row_id) {
    ...
    var be = document.getElementById('annotation_editor2');
    var new_row = annotation_editor_get_metadata_row_html2(row_id);
    var old_row = document.getElementById(new_row.getAttribute('id'));
    be.replaceChild(new_row, old_row);
}
...
function annotation_editor_add_row(row_id) {
    if (is_annotation_editor_visible()) {
        ...
        if (penultimate_row_id >= 0) {
            var penultimate_row_html_id = 'ae_' + 'region' + '_' + penultimate_row_id;
...
function annotation_editor_get_metadata_row_html2(row_id) {
    var row = document.createElement('div');
    row.setAttribute('class', 'row');
    row.setAttribute('id', 'be_' + 'file' + '_' + row_id);
    ...
    for (attr_id in _via_attributes['file']) {
        ...
        var attr_desc = _via_attributes['file'][attr_id].desc;
        ...
        switch (attr_type) {
            case 'text':
                col.innerHTML = '<textarea ' +
                    'onchange="annotation_editor_on_metadata_update2(this)" ' +
                ...
                break;
            case 'checkbox':
                var options = _via_attributes['file'][attr_id].options;
                var option_id;
                for (option_id in options) {
                    ...
                    option.setAttribute('onchange', 'annotation_editor_on_metadata_update2(this)');

                    var option_desc = _via_attributes['file'][attr_id].options[option_id];
            ...
            case 'radio':
                var option_id;
                for (option_id in _via_attributes['file'][attr_id].options) {
                    var option_html_id = attr_html_id + '__' + option_id;
                    var option = document.createElement('input');
            ...
            case 'dropdown':
                ...
                sel.setAttribute('onchange', 'annotation_editor_on_metadata_update2(this)');
                var option_id;
...
function annotation_editor_get_metadata_row_html(row_id) {
    var row = document.createElement('div');
    row.setAttribute('class', 'row');
    row.setAttribute('id', 'ae_' + 'region' + '_' + row_id);
    ...
    for (attr_id in _via_attributes['region']) {
        ...
        var attr_desc = _via_attributes['region'][attr_id].desc;
        if (typeof (attr_desc) === 'undefined') {
            attr_desc = '';
        }
        var attr_html_id = attr_id + '__' + row_id;
        ...
        switch (attr_type) {
            case 'text':
                ...
                break;
            case 'checkbox':
                var options = _via_attributes['region'][attr_id].options;
                var option_id;
                for (option_id in options) {
                    var option_html_id = attr_html_id + '__' + option_id;
                    var option = document.createElement('input');
                    ...
                    var option_desc = _via_attributes['region'][attr_id].options[option_id];
                    if (option_desc === '' || typeof (option_desc) === 'undefined') {
                        // option description is optional, use option_id when description is not present
                        option_desc = option_id;
                    }
            ...
            case 'radio':
                var option_id;
                for (option_id in _via_attributes['region'][attr_id].options) {
                    var option_html_id = attr_html_id + '__' + option_id;
                    var option = document.createElement('input');
                    option.setAttribute('type', 'radio');
                    option.setAttribute('name', attr_html_id);
...
function annotation_editor_clear_row_highlight() {
    if (is_annotation_editor_visible()) {
        ...
        var be = document.getElementById('annotation_editor2');
        var i;
        for (i = 0; i < be.childNodes.length; ++i) {
            be.childNodes[i].classList.remove('highlight');
        }
    }
}
...
function annotation_editor_on_metadata_update2(p) {
    var pid = annotation_editor_extract_html_id_components(p.id);
    var img_id = _via_image_id;
    var img_index_list = [_via_image_index];
    var region_id = pid.row_id;
...
function project_get_default_project_name() {
    ...
    var project_name = _via_project_name;
...
function project_save_confirmed(input) {
    ...
    if (!_via_attributes.region['골절 판독 구분']){
        alert('Please check 골절 판독 구분');
        return;
    }
...
function project_open_parse_json_file_execute(settings, attributes, img_metadata) {
    project_import_settings(settings);
    ...
    _via_settings.core.default_filepath = _via_direc_name;
    if (_via_settings.core.default_filepath !== '') {
...
function project_open_parse_json_file(project_file_data) {
    ...
    var project_name = project_get_default_project_name();
    if (d['_via_settings'] && d['_via_img_metadata'] && d['_via_attributes']) {
        // import settings
        if (project_name.includes('project')) {
            project_open_parse_json_file_execute(d['_via_settings'], d['_via_attributes'], d['_via_img_metadata'])
        } else {
            project_open_parse_json_file_execute(d['_via_attributes'], d['_via_settings'], d['_via_img_metadata'])
        }
    } else {
...
function project_file_add_local(event) {
    ...
    for (var i = 0; i < user_selected_images.length; ++i) {
        ...
        try {
            ...
            var number = _imgName.substring(1, 4);
            //if(filetype === 'image' && _imgName.length===6&&_imgName[0]==='R'&& pattern_eng.test(_imgName[4]) && !isNaN(Number(number))&& !isNaN(Number(_imgName[5]))){
            ...
            // wrong_img_list.push(user_selected_images[i].name);
            //}
        }
        catch (error) {
            alert(' Wrong file name pattern,\n Change file name Please.');
