...
function _via_draw_rect_sub_x(x, y, z, dx, dy, dz, temp, ratio, height_ratio, draw = true) {
    ...
    _via_reg_ctx2.strokeStyle = (draw===true) ? VIA_THEME_SEL_REGION_FILL_BOUNDARY_COLOR : VIA_THEME_BOUNDARY_FILL_COLOR;
    ...
    _via_reg_ctx3.strokeStyle = (draw===true) ? VIA_THEME_SEL_REGION_FILL_BOUNDARY_COLOR : VIA_THEME_BOUNDARY_FILL_COLOR;
...
function _via_draw_rect_sub_y(x, y, z, dx, dy, dz, temp, ratio, height_ratio, draw = true) {
    var _via_reg_ctx = _via_reg_canvas.getContext('2d');
    var _via_reg_ctx2 = _via_reg_canvas2.getContext('2d');
    var _via_reg_ctx3 = _via_reg_canvas3.getContext('2d');
...
function _via_draw_rect_sub_z(x, y, z, dx, dy, dz, temp, ratio, height_ratio, draw = true) {
    var _via_reg_ctx = _via_reg_canvas.getContext('2d');
    var _via_reg_ctx2 = _via_reg_canvas2.getContext('2d');
    var _via_reg_ctx3 = _via_reg_canvas3.getContext('2d');
    var height_ratio = (398.5 / 567);

    var current_viewer = papaya.Container.getObject(0).viewer;
...
function set_all_canvas_size(w, h) {
    ...
    console.log(w,h);
    var all_width = current_viewer.canvas.width;
    var all_height = current_viewer.canvas.height;
    var half_height = all_height/2;
    
    _via_reg_canvas.height = (h) * (0.7);
    _via_reg_canvas.width = (w * all_height / all_width) * (0.7);
    $('#region_canvas').css('margin-left', (w * all_height / all_width) * (0.15));
    $('#region_canvas').css('margin-right', (w * all_height / all_width) * (0.15));
    $('#region_canvas').css('margin-top', (w * all_height / all_width) * (0.15));
    $('#region_canvas').css('margin-bottom', (w * all_height / all_width) * (0.15));

    _via_reg_canvas2.height = (h ) / 2;
    _via_reg_canvas2.width = (w * half_height / all_width) * (0.7);
    $('#region_canvas2').css('margin-left', (w * half_height / all_width) * (0.15));
    $('#region_canvas2').css('margin-right', (w * half_height / all_width) * (0.15));

    _via_reg_canvas3.height = (h ) / 2 ;
    _via_reg_canvas3.width = (w * half_height / all_width) * (0.7);
    $('#region_canvas3').css('margin-left', (w * half_height / all_width) * (0.15));
    $('#region_canvas3').css('margin-right', (w * half_height / all_width) * (0.15));


    image_panel.style.height = h ;
...
function _via_draw_polygon_region(type, all_points_x, all_points_y, z, dz, is_selected, shape) {
    ...
    var ratio = (417 / 830);
    var temp = Number( $('#region_canvas2').attr('width') ) / (506);
    ...
    if (is_selected) {
        ...
        var cx = Math.round(current_viewer.currentCoord.x);
        var cy = Math.round(current_viewer.currentCoord.y);
        var cz = Math.round(current_viewer.currentCoord.z);
        ...
        if(case_slice === 'x'){
            ...
            var y = all_points_x.reduce((prev, current) => {return prev > current ? current : prev;});
            var dy = all_points_x.reduce((prev, current) => {return prev > current ? prev : current ;}) - y;
            var z = all_points_y.reduce((prev, current) => {return prev > current ? current : prev;});
            var dz = all_points_y.reduce((prev, current) => {return prev > current ? prev : current ;}) - z;
            
            _via_draw_rect_sub_x(x, y, z, dx, dy, dz, temp, ratio, height_ratio);

        }
        else if(case_slice === 'y'){
            if (Math.abs(cz - current_viewer.currentCoord.z) > dz) return;

            var x = all_points_x.reduce((prev, current) => {return prev > current ? current : prev;});
            var dx = all_points_x.reduce((prev, current) => {return prev > current ? prev : current ;}) - x;
            var z = all_points_y.reduce((prev, current) => {return prev > current ? current : prev;});
            var dz = all_points_y.reduce((prev, current) => {return prev > current ? prev : current ;}) - z;
            var y = z;
            var dy = dz;

            _via_draw_rect_sub_y(x, y, z, dx, dy, dz, temp, ratio, height_ratio);
        }
        else if(case_slice === 'z'){
            var height_ratio = (398.5 / 567);
            if (Math.abs(z - current_viewer.currentCoord.z) > dz) return;

            var x = all_points_x.reduce((prev, current) => {return prev > current ? current : prev;});
            var dx = all_points_x.reduce((prev, current) => {return prev > current ? prev : current ;}) - x;
            var y = all_points_y.reduce((prev, current) => {return prev > current ? current : prev;});
            var dy = all_points_y.reduce((prev, current) => {return prev > current ? prev : current ;}) - y;
            var z = z;
            var dz = dz;
            _via_draw_rect_sub_z(x, y, z, dx, dy, dz, temp, ratio, height_ratio);
        }
    } else {
        for (var i = 1; i < all_points_x.length; ++i) {
            // draw a fill line
            ...
            if (slope_i > 0) {
                ...
                if (case_slice === 'x') {
                    ...
                    _via_draw_rect_sub_x(x, y, z, dx, dy, dz, temp, ratio, height_ratio, false);
                    ...
                    _via_draw_rect_sub_y(x, y, z, dx, dy, dz, temp, ratio, height_ratio, false);
                }
                else if (case_slice === 'z') {
                    var height_ratio = (398.5 / 567);
                    if (Math.abs(z - current_viewer.currentCoord.z) > dz) return;
                    ...
                    var z = z;
                    var dz = dz;
                    _via_draw_rect_sub_z(x, y, z, dx, dy, dz, temp, ratio, height_ratio, false);
                }
    ...
    if (click_dx < VIA_MOUSE_CLICK_TOL ||
        ...
        if (_via_is_user_drawing_polygon) {
            ...
            if (Math.sqrt(dx * dx + dy * dy) <= VIA_POLYGON_VERTEX_MATCH_TOL &&
                ...
                polygon_region.shape_attributes['z'] = Math.round(current_viewer.currentCoord.z);
                polygon_region.shape_attributes['dz'] = 20;
                polygon_region.shape_attributes['type'] = case_slice;
                ...
                _via_canvas_regions[_via_current_polygon_region_id].shape_attributes['dz'] = 20;
                _via_canvas_regions[_via_current_polygon_region_id].shape_attributes['z'] = Math.round(current_viewer.currentCoord.z);;
        ...
        if (region_dx >= VIA_REGION_MIN_DIM && region_dy >= VIA_REGION_MIN_DIM) { // avoid regions with 0 dim
            switch (_via_current_shape) {
                ...
                case VIA_REGION_SHAPE.RECT:
                    ...
                    var x = Math.round(region_x0);
                    var y = Math.round(region_y0);
...
function _via_reg_canvas_mousemove_handler(e) {
    ...
    if (_via_is_user_drawing_region) {
        ...
        var region_y0 = (case_slice === 'z') ? _via_click_y0 : _via_click_y0;
            ...
            case VIA_REGION_SHAPE.RECT:
                if (_via_click_x0 < _via_current_x) {
                    if (_via_click_y0 < _via_current_y) {
                        region_x0 = _via_click_x0;
                        region_y0 = (case_slice === 'z') ? _via_click_y0 : _via_click_y0;
                    } else {
                        region_x0 = _via_click_x0;
                        region_y0 = (case_slice === 'z') ? _via_current_y : _via_current_y;
    ...
    if (_via_is_user_resizing_region) {
        ...
        switch (attr['name']) {
            ...
            case VIA_REGION_SHAPE.POLYGON:
                ...
                _via_draw_polygon_region(attr['type'],
                                        moved_all_points_x,
                                        moved_all_points_y,
                                        attr['z'],
                                        attr['dz'],
    ...
    if (_via_is_user_drawing_polygon) {
        ...
        if (npts > 0) {
            ...
            _via_draw_polygon_region(attr['type'], line_x, line_y, attr['z'], attr['dz'], false);
        }
...
function _via_draw_control_spoint_sub(cx, cy, n) {
    ...
    _via_sub_ctxt.arc(cx, cy, 1, 0, 2 * Math.PI, false);
    _via_sub_ctxt.closePath();

    _via_sub_ctxt.fillStyle = VIA_THEME_CONTROL_POINT_COLOR;
    _via_sub_ctxt.globalAlpha = 1.0;
    _via_sub_ctxt.fill();
}
...
async function _via_toggle_labelling_view() {
    var case_slice = $('#papayaContainer0').attr('slice');
    var main_canvas_width = _via_coronal_x;
    var main_canvas_height = _via_all_height;
    var sub_canvas_width = _via_all_width - _via_coronal_x;
    var sub_canvas_height = _via_coronal_y;
    ...
    if (_via_zoom_degree === 1) {
        if (case_slice === 'x') {
            ...
            var ratios = sub_canvas_height / main_canvas_height;
            ...
            await $('#region_canvas2').css('margin-top', temp_margin * ratios);
            await $('#region_canvas2').css('margin-bottom', temp_margin * ratios);
            await $('#region_canvas2').attr('height', Math.round(Number($('#region_canvas2').attr('height')) * (0.7)));
            await $('#region_canvas2').css('height', Number(_via_reg_canvas2.height));
        }

        else if (case_slice === 'z') {
            var ratios =  main_canvas_height / sub_canvas_height;
            ...
            await $('#region_canvas3').attr('height', Number($('#region_canvas3').css('height').split('px')[0]));

            await $('#region_canvas').css('margin-top', Number(temp_margin * ratios));
            await $('#region_canvas').css('margin-bottom', Number(temp_margin * ratios));
            await $('#region_canvas').attr('height', Number($('#region_canvas').css('height').split('px')[0] * (0.7)));
...
async function _via_draw_cube_region(x, y, z, dx, dy, dz, cx, cy, cz, degree, is_selected) {
    ...
    if (is_selected) {
        if (case_slice === 'x') {
            ...
            _via_draw_control_point(y, _z);
            _via_draw_control_point(y + dy, _z + dz);
            _via_draw_control_point(y, _z + dz);
            _via_draw_control_point(y + dy, _z);
            _via_draw_control_point(y + dy / 2, _z);
            _via_draw_control_point(y + dy / 2, _z + dz);
            _via_draw_control_point(y, _z + dz / 2);
            _via_draw_control_point(y + dy, _z + dz / 2);
            ...
            _via_draw_control_point(x, _z);
            _via_draw_control_point(x + dx, _z + dz);
            _via_draw_control_point(x, _z + dz);
            _via_draw_control_point(x + dx, _z);
            _via_draw_control_point(x + dx / 2, _z);
            _via_draw_control_point(x + dx / 2, _z + dz);
            _via_draw_control_point(x, _z + dz / 2);
            _via_draw_control_point(x + dx, _z + dz / 2);
    ...
    switch (shape) {
        ...
        case 'cube':
            ...
            if (slice === 'x') {
                ...
                html_position.left = r['y'] + r['dy'] + Number(margins.split('p')[0]);
            }
            else if (slice === 'y') {
                html_position.top = r['z'] + r['dz'];
...
function via_canvas_toggle(shape = 'cube') {
